                ...
                {!loading && !error && (
                    <>
                        <div style={styles.resumen}>
                            <div style={styles.stat}><span>Obj</span><strong>{bloques.length}</strong><small>Total objetos</small></div>
                            <div style={styles.stat}><span>Coin</span><strong>{coins.length}</strong><small>Coins</small></div>
                            <div style={styles.stat}><span>Portal</span><strong>{portal.length}</strong><small>Portal</small></div>
                            <div style={styles.stat}><span>Bloq</span><strong>{objetos.length}</strong><small>Bloques</small></div>
                        </div>
                        ...
                        {objetos.length > 0 && (
                            <>
                                ...
                                <div style={styles.tabla}>
                                    ...
                                    {objetos.length > 20 && (
                                        <p style={styles.info}>... y {objetos.length - 20} objetos mas</p>
                                    )}
                                </div>
                            </>
                        )}
...
const styles = {
    container: { position: 'fixed', inset: 0, width: '100vw', height: '100vh', overflowY: 'auto', background: 'linear-gradient(135deg,#0f0c29,#302b63,#24243e)', fontFamily: 'sans-serif', padding: '32px 16px', boxSizing: 'border-box' },
    wrapper: { maxWidth: 760, margin: '0 auto', paddingBottom: 80 },
